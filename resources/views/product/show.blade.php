
<x-layout>

    <section class="px-6 py-8">
        <main class="max-w-6xl mx-auto mt-10 lg:mt-20 space-y-6">
            <article class="max-w-4xl mx-auto lg:grid lg:grid-cols-12 gap-x-10">
                <div class="col-span-4 lg:text-center lg:pt-14 mb-10">
                    <img src="{{asset('storage/'.$product->image)}}" alt="{{$product->name}}" class="rounded-xl" width="200px" height="300px">


                    <div class="flex items-center lg:justify-center text-sm mt-4">

                        <div class="ml-3 text-left">
                            seller{{-- <a href="/?auther={{ $product->user->name }}"><h5 class="font-bold">{{ $post->user->name }}</h5></a> --}}

                        </div>
                    </div>
                </div>

                <div class="col-span-8">
                    <div class="hidden lg:flex justify-between mb-6">
                        <a href="/"
                            class="transition-colors duration-300 relative inline-flex items-center text-lg hover:text-blue-500">
                            <svg width="22" height="22" viewBox="0 0 22 22" class="mr-2">
                                <g fill="none" fill-rule="evenodd">
                                    <path stroke="#000" stroke-opacity=".012" stroke-width=".5" d="M21 1v20.16H.84V1z">
                                    </path>
                                    <path class="fill-current"
                                        d="M13.854 7.224l-3.847 3.856 3.847 3.856-1.184 1.184-5.04-5.04 5.04-5.04z">
                                    </path>
                                </g>
                            </svg>

                            Back to Main page
                        </a>

                        <div class="space-x-2">
                            <x-category-button :category="$product->category" />

                        </div>
                    </div>

                    <h1 class="font-bold text-3xl lg:text-4xl mb-10">
                        {{$product->name}}
                    </h1>

                    <div class="space-y-4 lg:text-lg leading-loose">
                        <p>{{$product->description}}</p>

                    </div>
                    <h1 class="mt-4 block text-gray-400 mb-24">
                        Price {{$product->price}}$.
                    </h1>
                    <div class="ml-34">
                        @auth
                        <form action="/addToCart" method="POST">
                            @csrf
                            @php @endphp
                            <input type="hidden" value=""  name="cart_id">
                            <input type="hidden" value="{{auth()->user()->id}}" name="customer_id">
                            <input type="hidden" value="{{$product->id}}" name="product_id">
                            <input type="hidden" value=1 name="quantity">
                            <input type="hidden" value="{{$product->price}}" name="price">
                            <button type="submit" class="bg-blue-400 text-white rounded py-2 px-4 hover:bg-500">Add To Cart</button>
                        </form>
                    @endauth
                    </div>

                </div>
                {{-- <section class="col-span-8 col-start-5 mt-10 space-y-6">
                    @auth
                        <form method="POST" action="/posts/{{$product->name}}/comments" class="border border-gray-200 p-6 rounded-xl">
                            @csrf

                            <header class="flex items-center">
                                <img src="https://i.pravatar.cc/60?u={{ auth()->user()->id }}" alt="" width="40" height="40" class="rounded-full">
                                <h2 class="ml-4">Want to participate? </h2>
                            </header>
                            <div class="mt-6">
                                <textarea name="body" class="p-2 w-full text-xs focus:outline-none focus:ring" rows="5" placeholder="Quick, thing of something to say!" required></textarea>
                                @error('body')
                                    <span class="text-xs text-red-500">{{$message}}</span>
                                @enderror
                            </div>
                            <div class="flex justify-end mt-6 pt-6 border-t border-gray-200">
                                <button type="submit" class="bg-blue-500 text-white uppercase font-semibold text-xs py-2 px-10 rounded-2xl hover:bg-blue-600">Psot</button>
                            </div>

                        </form>
                    @else
                        <p class="font-bold">
                            <a href="/register" class="hover:underline">Register</a> Or <a href="/login" class="hover:underline">Log in</a> to leave a comment!
                        </p>
                    @endauth

                    @foreach ($post->comments as $comment )

                            <article class="flex bg-gray-100 border border-gray-200 p-6 rounded-xl space-x-4">
                                <div class="flex-shrink-0">
                                    <img src="https://i.pravatar.cc/60?u={{ $comment->id }}" alt="" width="60" height="60" class="rounded-xl">
                                </div>
                                <div>
                                    <header class="mb-4">
                                        <h3 class="font-blod">{{$comment->user->name}}</h3>

                                        <p class="text-xs">
                                            Posted <time>{{$comment->created_at->diffForHumans()}}</time>
                                        </p>

                                    </header>

                                    <p>
                                        {{$comment->body}}
                                    </p>
                                </div>
                            </article>
                    @endforeach
                    <x-post-comment />

                </section>

            </article>
        </main>
    </section> --}}
</x-layout>

{{-- @endsection --}}
