                        ...
                        @if ($cartItems->count())
                          @foreach ($cartItems as $cartItem)
                            ...
                            <td class="hidden pb-4 md:table-cell">
                                @php $path =asset('storage/'.$cartItem->product->image);@endphp
                                <a href="/product/{{$cartItem->product->name}}">
                                <img src="{{ $path }}" class="w-20 rounded" alt="Thumbnail">
                                </a>
                            </td>
                            <td>
                                <a href="/product/{{$cartItem->product->name}}">
                                <p class="mb-2 md:ml-4">{{ $cartItem->product->name }}</p>
                            ...
                            <td class="justify-center mt-6 md:justify-end md:flex">
                                ...
                                <div class="relative flex flex-row w-full h-8">
                                    ...
                                    <form action="/cart/adjustAmount" method="POST">
                                        ...
                                        <input type="hidden" value="{{$cartItem->product->price}}" name="price">
                                        <output>{{$cartItem->quantity}}</output>

                                        <input type="hidden" name="totalQuantity" value="{{$cartItem->product->quantity}}">
